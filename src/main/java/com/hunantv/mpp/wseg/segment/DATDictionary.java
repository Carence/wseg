...
import com.hunantv.mpp.wseg.recognition.PersonAttrLibrary;
import com.hunantv.mpp.wseg.recognition.PersonNatureAttr;
...
public class DATDictionary {
	...
	private static DoubleArrayTire loadDAT() {
		...
		try {
			...
			DoubleArrayTire dat = DoubleArrayTire.loadText(DicReader.getInputStream("core.dic"), MyItem.class);
			...
			MyStaticValue.logger.info("init core library ok use time :" + (System.currentTimeMillis() - start));
	...
	private static void personNameFull(DoubleArrayTire dat) throws NumberFormatException, IOException {
		...
		for (Entry<String, PersonNatureAttr> entry : entrySet) {
			...
			if (temp.length() == 1 && (ansjItem = (MyItem) dat.getDAT()[temp.charAt(0)]) == null) {
				ansjItem = new MyItem();
				ansjItem.setBase(c);
				ansjItem.setCheck(-1);
