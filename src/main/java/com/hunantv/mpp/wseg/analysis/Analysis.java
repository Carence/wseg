...
import com.hunantv.mpp.wseg.dict.GetWordsImpl;
...
import com.hunantv.mpp.wseg.segment.TermNature;
...
public abstract class Analysis {
		...
		for (int i = startOffe; i < endOffe; i++) {
			switch (DATDictionary.status(chars[i])) {
				...
				default:
					 ...
					 * 如果未分出词.以未知字符加入到gp中
					 */
					if (DATDictionary.IN_SYSTEM[c] > 0 || DATDictionary.status(c) > 3) {
						i -= 1;
					} else {
						gp.addTerm(new Term(String.valueOf(c), i, TermNatures.NULL));
					}
	 ...
	 * @param br
	 */
	public void resetContent(MyReader br) {
		this.offe = 0;
		this.br = br;
	}
	...
	public void resetContent(Reader reader) {
		this.offe = 0;
		this.br = new MyReader(reader);
	}

	public void resetContent(Reader reader, int buffer) {
		this.offe = 0;
		this.br = new MyReader(reader, buffer);
	}
