...
public class UserDefineLibrary {
	...
	protected static final Logger logger = Logger.getLogger(UserDefineLibrary.class);
	...
	private static void initAmbiguityLibrary() {
		...
		if (StringUtil.isBlank(ambiguityLibrary)) {
			logger.warn("init ambiguity  warning :" + ambiguityLibrary + " because : file not found or failed to read !");
			return;
		}
		...
		if (file.isFile() && file.canRead()) {
			try {
				...
				logger.warn("init ambiguity  error :" + new File(ambiguityLibrary).getAbsolutePath() + " because : not find that file or can not to read !");
				e.printStackTrace();
			}
			logger.info("init ambiguityLibrary ok!");
		} else {
			logger.warn("init ambiguity  warning :" + new File(ambiguityLibrary).getAbsolutePath() + " because : file not found or failed to read !");
		}
	}
	...
	public static void loadLibrary(Forest forest, String path) {
		...
		if (path != null) {
			...
			if (!file.canRead() || file.isHidden()) {
				logger.warn("init userLibrary  warning :" + new File(path).getAbsolutePath() + " because : file not found or failed to read !");
				return;
			}
			if (file.isFile()) {
				...
				logger.warn("init user library  error :" + new File(path).getAbsolutePath() + " because : not find that file !");
			}
		}
	}
